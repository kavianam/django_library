...
class LoanedBookInstancesByUserListViewTest(TestCase):
    ...
    def test_redirect_if_not_logged_in(self):
        response = self.client.get(reverse('user_borrowed_books'))
        self.assertRedirects(response, '/accounts/login/?next=/catalog/my-borrowed-books/')
    ...
    def test_logged_in_uses_correct_template(self):
        ...
        self.assertTemplateUsed(response, 'catalog/user_borrowed_books.html')
...
class AuthorCreateTest(TestCase):
    def setUp(self) -> None:
        user1 = User.objects.create_user('user1', password='2HJ1vRV0Z&3iD')
        user2 = User.objects.create_user('user2', password='3HJ1vRV0Z&3iD')

        permission = Permission.objects.get(name="Set book as returned")
        user1.user_permissions.add(permission)

    def test_not_login_redirect_login(self):
        response = self.client.get(reverse('author-create'))

        self.assertRedirects(response, reverse('login') + '?next=' + reverse('author-create'))

    def test_permission(self):
        login = self.client.login(username='user2', password='3HJ1vRV0Z&3iD')
        response = self.client.get(reverse('author-create'))
    ...
    def test_redirect(self):
        ...
        response = self.client.post(reverse('author-create'),
                                    {'first_name': 'ali', 'last_name': 'mozafari'})
